<template>
  <div class="rsa-workbench">
    <div class="title">RSA 工作台</div>
    <el-tabs v-model="tab">
      <el-tab-pane label="密钥运算" name="key">
        ...
        <el-row :gutter="24" style="margin-top:16px">
          <el-col :span="12">
            <div class="label">密钥长度</div>
            <el-select v-model="ko.keySize" style="width:100%">
      ...
      <el-tab-pane label="签名验签" name="sign">
        ...
        <el-row :gutter="24" style="margin-top:16px">
          ...
          <el-col :span="12">
            ...
            <el-input v-model="sg.signature" type="textarea" :rows="4" placeholder="请输入对应数据格式的数据" />
          </el-col>
        </el-row>
        <el-row :gutter="24" style="margin-top:16px">
          <el-col :span="12">
            <div class="label">签名算法</div>
            <el-select v-model="sg.algorithm" style="width:100%">
              <el-option label="MD2withRSA" value="MD2withRSA" />
              ...
              <el-option label="SHA3-224withRSA" value="SHA3-224withRSA" />
              <el-option label="SHA3-256withRSA" value="SHA3-256withRSA" />
              <el-option label="SHA3-384withRSA" value="SHA3-384withRSA" />
              <el-option label="SHA3-512withRSA" value="SHA3-512withRSA" />
            </el-select>
          </el-col>
        </el-row>
        <div class="actions">
      ...
      <el-tab-pane label="加密解密" name="enc">
        ...
        <el-row :gutter="24" style="margin-top:16px">
          <el-col :span="12">
            ...
            <el-select v-model="en.padding" style="width:100%">
              <el-option label="RSA/ECB/PKCS1Padding (标准填充)" value="RSA/ECB/PKCS1Padding" />
              <el-option label="RSA/ECB/NoPadding (无填充)" value="RSA/ECB/NoPadding" />
              <el-option label="RSA/ECB/OAEPPadding (默认SHA-1)" value="RSA/ECB/OAEPPadding" />
              <el-option label="RSA/ECB/OAEPWithSHA-1AndMGF1Padding" value="RSA/ECB/OAEPWithSHA-1AndMGF1Padding" />
              <el-option label="RSA/ECB/OAEPWithSHA-256AndMGF1Padding" value="RSA/ECB/OAEPWithSHA-256AndMGF1Padding" />
              <el-option label="RSA/ECB/OAEPWithSHA-384AndMGF1Padding" value="RSA/ECB/OAEPWithSHA-384AndMGF1Padding" />
              <el-option label="RSA/ECB/OAEPWithSHA-512AndMGF1Padding" value="RSA/ECB/OAEPWithSHA-512AndMGF1Padding" />
              <el-option label="RSA/ECB/OAEPWithMD5AndMGF1Padding" value="RSA/ECB/OAEPWithMD5AndMGF1Padding" />
            </el-select>
          </el-col>
        </el-row>
        <div class="actions">
...
function stripWhitespaceSign() {
  sg.publicKey = strip(sg.publicKey)
  sg.privateKey = strip(sg.privateKey)
  sg.message = strip(sg.message)
  sg.signature = strip(sg.signature)
}
function clearSign() {
  sg.publicKey = ''; sg.privateKey = ''
  sg.message = ''; sg.signature = ''
  sg.result = ''
}
...
async function genKey() {
  const r = await run(() => api.post('/rsa/keypair', { keySize: ko.keySize }))
...
async function doSign() {
  ...
  const r = await run(() => api.post('/rsa/sign', {
...
async function doVerify() {
  ...
  const r = await run(() => api.post('/rsa/verify', {
...
async function doEncrypt() {
  ...
  const r = await run(() => api.post('/rsa/encrypt', {
...
async function doDecrypt() {
  ...
  const r = await run(() => api.post('/rsa/decrypt', {
